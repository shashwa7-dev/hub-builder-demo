...
function WorkflowBuilderInner({
  ...
  const handleSaveWorkflowImmediate = useCallback(() => {
    ...
    console.log(
      "💾 [MANUAL SAVE] Saving workflow with",
      nodes.length,
      "nodes and",
      edges.length,
      "edges"
  ...
  useEffect(() => {
    if (workflow) {
      ...
      if (warnings.length > 0 && duplicateWarnings.length === 0) {
        toast.warning(
          `Found ${warnings.length} duplicate credential${
            warnings.length > 1 ? "s" : ""
          }`,
          {
  ...
  useEffect(() => {
    console.log(
      "🎨 Visual update triggered - currentNodeId:",
      currentNodeId,
      "nodeResults:",
    ...
    setNodes((nds) =>
      nds.map((node) => {
        ...
        const isRunningChanged =
          (node.data as any)?.isRunning !== isCurrentlyRunning;
        const statusChanged =
          ...
          return {
            ...
            data: {
              ...
              onUpdate: (updates: any) =>
                handleUpdateNodeData(node.id, updates),
            },
          };
        }
        return node;
          ...
          >
            ...
            <div className="p-24">
              ...
              <p className="text-body-medium text-black-alpha-64 mb-20">
                The File Search node will allow you to search through files and
                code in your workflows. Stay tuned for updates!
              </p>
