  ...
  useEffect(() => {
    Object.entries(nodeResults).forEach(([nodeId, result]) => {
      if (
        ...
        const isGoogleDocResult =
          outputObj?.result?.output?.value?.documentUrl ||
        ...
        if (isGoogleDocResult) {
          ...
          if (outputObj?.result?.output?.value?.documentUrl) {
            ...
            docTitle =
              outputObj.result.output.value.title || "Executive Summary";
          } else if (outputStr.includes("documentUrl")) {
            try {
              const parsed = JSON.parse(outputStr);
              docUrl =
                parsed.result?.output?.value?.documentUrl || parsed.documentUrl;
              docTitle =
                parsed.result?.output?.value?.title ||
  ...
  useEffect(() => {
    if (inputVariables.length > 0) {
      const defaults = inputVariables.reduce((acc: any, v: any) => {
        acc[v.name] =
          v.defaultValue || (v.type === "url" ? "https://firecrawl.dev" : "");
        return acc;
      }, {});
    ...
    const results = {
      ...
      dataFlow: {
        nodes: Object.entries(nodeResults).map(([nodeId, result]) => {
          ...
          const outgoingEdges =
            workflow?.edges.filter((e) => e.source === nodeId) || [];
          const outgoingConnections = outgoingEdges.map((edge) => {
            const targetNode = workflow?.nodes.find(
              (n) => n.id === edge.target
            );
            const targetNodeName =
              (targetNode?.data as any)?.nodeName || edge.target;
            return {
        ...
        connections:
          workflow?.edges.map((edge) => ({
            ...
            fromNodeName:
              (workflow.nodes.find((n) => n.id === edge.source)?.data as any)
                ?.nodeName || edge.source,
            toNodeName:
              (workflow.nodes.find((n) => n.id === edge.target)?.data as any)
      ...
      ? inputVariables.some(
          (variable: any) =>
            ...
            (inputValues[variable.name] === undefined ||
              ...
              inputValues[variable.name] === null)
        )
      : false;

  const canTriggerShortcut =
    showInput && !isRunning && !hasMissingRequiredInputs;
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="p-20 border-b border-border-faint flex-shrink-0">
                  ...
                  <button
                    onClick={() => {
                      const results = {
                        ...
                        inputs: inputValues,
                        dataFlow: {
                          nodes: Object.entries(nodeResults).map(
                            ([nodeId, result]) => {
                              ...
                              return {
                                ...
                                nodeName,
                                nodeType:
                                  (node?.data as any)?.nodeType || node?.type,
                                status: result.status,
                                incomingConnections: incomingEdges.map(
                                  (edge) => ({
                                    fromNodeId: edge.source,
                                    fromNodeName:
                                      (
                                ...
                                output: result.output,
                                toolCalls: result.toolCalls || [],
                                outgoingConnections: outgoingEdges.map(
                                  (edge) => ({
                                    toNodeId: edge.target,
                                    toNodeName:
                                      (
          ...
          {execution?.status === "waiting-auth" &&
            pendingAuth &&
            pendingAuth.toolName === "user-approval" && (
                ...
                <div className="flex flex-col gap-12">
                  <div>
                    ...
                    <p className="text-body-small text-black-alpha-64 mt-4">
                      {pendingAuth.message ||
                        "Open the authorization link in a new tab, approve access, then resume the workflow."}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-10 items-center">
                    ...
                    >
                      ...
                      <span>
                        {isResumingAuth ? "Resuming…" : "Resume workflow"}
                      </span>
                    </button>
                  </div>
                </div>
                ...
                <div className="inline-flex items-center gap-8 px-12 py-6 rounded-8 text-body-small bg-black-alpha-4 border border-border-faint">
                  ...
                  <div className="flex items-center gap-6">
                    <NodeIcon
                      ...
                      strokeWidth={2.5}
                    />
                    <span className="text-accent-black font-medium">
          ...
          {showInput ? (
            <div className="p-20">
              ...
              {inputVariables.length > 0 ? (
                <div className="mb-24 space-y-16">
                  <h3 className="text-label-medium text-accent-black">
                    Workflow Inputs
                  </h3>
              ...
              {workflow && (
                <div className="mt-24 p-16 bg-accent-white rounded-12 border border-border-faint">
                  <h3 className="text-label-medium text-accent-black mb-12">
                    Workflow Summary
                  </h3>
            ...
            <div className="p-20">
              ...
              <div className="mb-24 p-16 bg-accent-white rounded-12 border border-border-faint">
                <p className="text-body-small text-accent-black mb-4 font-medium">
                  Inputs:
                </p>
                {inputVariables.length > 0 ? (
                  ...
                  <p className="text-body-medium text-accent-black">
                    {inputValues.input}
                  </p>
                )}
              </div>
              ...
              {isRunning && Object.keys(nodeResults).length === 0 ? (
                ...
                <div className="space-y-16">
                  {Object.entries(nodeResults).map(([nodeId, result]) => {
                    ...
                    const nodeData = node?.data as any;
                    const nodeName =
                      nodeData?.nodeName || nodeData?.label || "Node";
                    const nodeType =
                      nodeData?.nodeType || node?.type || "agent";
                    const NodeIcon = getNodeIcon(nodeType);
                    const nodeColor = getNodeColor(nodeType);
                    const isActive =
                      currentNodeId === nodeId && result.status === "running";
                    const statusLabel =
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mb-12">
                          ...
                          <div className="flex items-center gap-8">
                            ...
                            {result.status !== "completed" && (
                              <span
                                className={`text-body-small px-8 py-4 rounded-6 border ${
                                  result.status === "running"
                                    ? "bg-accent-white text-black-alpha-64 border-border-faint"
                                    : result.status === "failed"
                                      ? "bg-accent-white text-accent-black border-border-faint"
                        ...
                        {expandedSchemas.has(nodeId) && (
                          ...
                          >
                            <div className="flex items-center justify-between mb-8">
                              <h4 className="text-label-small text-accent-black font-medium">
                                Data Flow Schema
                              </h4>
                              <button
                                onClick={() => {
                                  const schema = {
                                    ...
                                    incomingConnections: (
                                      ...
                                      return {
                                        from:
                                          (sourceNode?.data as any)?.nodeName ||
                                          edge.source,
                                        label: edge.label,
                                    ...
                                    outgoingConnections: (
                                      ...
                                      return {
                                        to:
                                          (targetNode?.data as any)?.nodeName ||
                                          edge.target,
                                        label: edge.label,
                                      };
                                    }),
                            ...
                            <div className="space-y-12">
                              ...
                              {(() => {
                                ...
                                if (incomingEdges.length > 0) {
                                  return (
                                    <div>
                                      <p className="text-body-small text-black-alpha-48 mb-6 font-medium">
                                        Incoming Data:
                                      </p>
                                      {incomingEdges.map((edge) => {
                                        ...
                                        const sourceName =
                                          ...
                                          edge.source;
                                        const sourceOutput =
                                          nodeResults[edge.source]?.output;
                                        return (
                                          <div
                                            key={edge.id}
                                      ...
                                      {result.toolCalls.map((call, index) => (
                                        ...
                                        >
                                          <div className="flex items-center justify-between mb-6">
                                            ...
                                            <span className="text-xs text-black-alpha-32">
                                              #{index + 1}
                                            </span>
                                          </div>
                                          {call.arguments && (
                                            <div className="mb-6">
                                              <p className="text-[10px] text-black-alpha-48 mb-2 uppercase tracking-wide">
                                                Arguments
                                              </p>
                        ...
                        {result.error && (
                          <div className="mt-12">
                            <div className="flex items-center justify-between mb-6">
                              <p className="text-body-small text-accent-black font-medium">
                                Error:
                              </p>
                              <button
                                onClick={() =>
                                  handleCopyError(result.error!, nodeId)
                                }
                        ...
                        {result.pendingAuth && (
                          <div className="mt-12 p-12 bg-heat-4 border border-heat-100 rounded-8">
                            ...
                            <p className="text-body-small text-black-alpha-64 mt-4">
                              {result.pendingAuth.message ||
                                "Complete authorization to allow the workflow to continue from this node."}
                            </p>
                        ...
                        {result.output && !result.error && (
                          <div className="mt-12">
                            <p className="text-body-small text-black-alpha-48 mb-6">
                              Output:
                            </p>
                              ...
                              if (isGoogleDocResult) {
                                ...
                                if (
                                  ...
                                  docUrl =
                                    outputObj.result.output.value.documentUrl;
                                  docTitle =
                                    outputObj.result.output.value.title ||
                                    "Executive Summary";
                                } else if (outputStr.includes("documentUrl")) {
                                  try {
                                    const parsed = JSON.parse(outputStr);
                                    docUrl =
                                      parsed.result?.output?.value
                                    ...
                                    <div className="space-y-12">
                                      ...
                                      <div className="bg-background-base border border-border-faint rounded-8 p-12">
                                        ...
                                        <p className="text-body-small text-black-alpha-48 mb-12">
                                          Your executive summary has been
                            ...
                            <div className="space-y-8">
                              {result.toolCalls.map((call, index) => (
                                ...
                                >
                                  ...
                                  {call.arguments && (
                                    <div className="mb-8">
                                      <p className="text-[11px] text-black-alpha-48 mb-2 uppercase tracking-wide">
                                        Arguments
                                      </p>
                    ...
                    >
                      ...
                      <div className="mb-12">
                        <div className="flex items-center justify-between mb-6">
                          ...
                          <button
                            onClick={() =>
                              handleCopyError(execution.error!, "workflow")
                            }
